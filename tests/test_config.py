...
def test_load_valid_config_and_checksum(scenario_yaml):
    ...
    assert scenario.checksum() == ScenarioConfig.from_mapping(scenario.raw_data, source_path=scenario_yaml).checksum()
...
def test_offset_array_expansion(scenario_yaml):
    ...
    assert scenario.array.rx_positions[0, 0] == pytest.approx(scenario.array.tx_positions[0, 0] + 0.01)
